var Carousela7laSlider;
(function ($) {
    Carousela7laSlider = function (element, options) {
        var settings = $.extend({}, $.fn.Carousela7la.defaults, options),
            self = this,
            element = $(element),
            Carousela7la = element.children('.slidesa7la');
        Carousela7la.children('div').addClass('slideItem');
        var slideItems = Carousela7la.children('.slideItem'),
        ...
        var horizonPos = function (pos) {
                ...
                if (option.hAlign == 'center') {
                    if (pos == 0) {
                        hPos = (option.Carousela7laWidth - option.frontWidth) / 2;
                    } else if (pos > 0 && pos <= Math.ceil((numberSlides - 1) / 2)) {
                        hPos = horizonPos(pos - 1) - hm;
                        if (mod == 0) {
                            if (pos == endSlide) {
                                hPos = (option.Carousela7laWidth - setSlideSize(pos).width) / 2;
                            }
                        }
                    } else {
                        hPos = (option.Carousela7laWidth) - (horizonPos(numberSlides - pos)) - setSlideSize(pos).width;
                    }
                } else {
                    if (pos == 0) {
                        hPos = option.left;
        ...
        var buildVideoOverlay = function () {
                ...
                $('<div class="videoOverlay"></div>').hide().css(setSlidePosition(0)).css({
                    ...
                    'margin': '0',
                    'border': 'none'
                }).bind('click', videoOverlayClick).appendTo(Carousela7la);
            };
        var showVideoOverlay = function (index) {
                if (slideItems.eq(index).children('a').hasClass('video')) {
                    Carousela7la.children('.videoOverlay').show();
                }
            };
        var hideVideoOverlay = function () {
                var car = Carousela7la;
        ...
        var addVideoContent = function () {
                var vc = Carousela7la.children('.videoOverlay'),
        ...
        var videoOverlayClick = function (event) {
                addVideoContent();
                Carousela7la.children('.spinner').show();
                $(this).hide();
        ...
        var buildCarousela7la = function () {
                Carousela7la.css({
                    'width': settings.Carousela7laWidth + 'px',
                    'height': settings.Carousela7laHeight + 'px'
                });
                for (var i = 0; i < numberSlides; i++) {
                    var item = slideItems.eq(i);
        ...
        var Carousela7laMouseOver = function (event) {
                var option = settings;
                if (option.autoplay == true && option.pauseOnHover == true) {
                    stopAutoplay();
                }
            };
        var Carousela7laMouseOut = function (event) {
                var option = settings;
                if (option.autoplay == true && option.pauseOnHover == true) {
                    if (pause == true) {
                        runAutoplay();
        ...
        var initialize = function () {
                ...
                Carousela7la.bind('mouseover', Carousela7laMouseOver).bind('mouseout', Carousela7laMouseOut);
                if (settings.mouse == true) {
                    Carousela7la.mousewheel(function (event, delta) {
                        if (delta > 0) {
                            prevClick();
                            return false;
                        } else if (delta < 0) {
    ...
    $.fn.Carousela7la = function (options) {
        var defaults = {
            hAlign: 'center',
            vAlign: 'center',
            hMargin: 0.4,
            ...
            frontWidth: 400,
            frontHeight: 300,
            Carousela7laWidth: 1000,
            Carousela7laHeight: 360,
            left: 0,
            right: 0,
            top: 0,
            bottom: 0,
            ...
            before: function (Carousela7la) {},
            after: function (Carousela7la) {}
        };
        var options = $.extend(defaults, options);
        var returnArr = [];
        for (var i = 0; i < this.length; i++) {
            if (!this[i].Carousela7la) {
                this[i].Carousela7la = new Carousela7laSlider(this[i], options);
            }
            returnArr.push(this[i].Carousela7la);
        }
